        ...
        for passport in puzzle_input:
            ...
            try:
                if not (passport.keys() >= {'byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid'}):
                    pass
                elif not (1920 <= int(passport['byr']) <= 2002 and 2010 <= int(passport['iyr']) <= 2020 and 2020 <= int(passport['eyr']) <= 2030):
                    pass
                elif not ((passport['hgt'][-2:] == 'cm' and 150 <= int(passport['hgt'][:-2]) <= 193) or (passport['hgt'][-2:] == 'in' and 59 <= int(passport['hgt'][:-2]) <= 76)):
                    pass
                elif not (passport['hcl'][0] == '#' and len(passport['hcl'][1:]) == 6 and all(char in hair_colors for char in passport['hcl'][1:])):
                    pass
                elif not (passport['ecl'] in eye_colors):
                    pass
                elif not (int(passport['pid']) and len(passport['pid']) == 9):
                    pass
                else:
                    valid_count += 1
            except:
